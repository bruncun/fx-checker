...
type RateHistoryRangeViewerProps = {
  panels: RateHistoryRangePanel[];
  selectedRange: HistoryRange;
};

function RateHistoryRangeViewer({ panels, selectedRange }: RateHistoryRangeViewerProps) {
  const pathname = usePathname() ?? "/";
  const searchParams = useSearchParams();
  const searchParamsString = searchParams.toString();
  const [currentRange, setCurrentRange] = useState(selectedRange);
  const selectedPanel = useMemo(
    () => panels.find((panel) => panel.range === currentRange) ?? panels[0],
    [currentRange, panels]
  );
...
export type { RateHistoryRangePanel };
